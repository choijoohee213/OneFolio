...
export function AccountsPanel({ accounts, coveredAsset, totalAsset, busy, onRemove }: Props) {
    ...
    <section className="accounts">
      {missing.length > 0 && (
        <p className="notice">
          ...
          올린 계좌({won(coveredAsset)})만 기준입니다. 나머지 계좌 파일을 추가로 올리면 합쳐서
          집계합니다 — {missing.map((account) => account.type).join(', ')}.
        </p>
      )}
